...
function Strategy(options, verify) {
  ...
  if (!options.wordpressUrl) throw new Error('OAuth2-Complete-For-Wordpress requires a wordpressUrl option');

  options.authorizationURL = options.authorizationURL || options.wordpressUrl + '/oauth/authorize';
  options.tokenURL = options.tokenURL || options.wordpressUrl + '/oauth/request_token';
  this.profileUrl = options.profileUrl || options.wordpressUrl + '/oauth/request_access/'; // Note: Must add slash at the end, otherwise 301 will be returned
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this.profileUrl, accessToken, function (err, body, res) {      
    ...
    try {
      ...
      var profile = { provider: 'OAuth2-Complete-For-WordPress' };
      profile.id = json.ID;
...
/**
 * Override authorizationParams to add state to params to overcome mandatory state parameter
 * in OAuth2 Complete For WordPress plugin.
 *
 * @param options
 * @returns {{}}
 */
Strategy.prototype.authorizationParams = function(options) {
  ...
  {
    params[ keys[l-1] ] = options[ keys[l-1] ];
  }

  // add underscore because the OAuth2 for WordPress plugin require this param.
  if (!params.state) params.state = '_';
